  ...
  <div class="admin-dashboard">
    ...
    <div class="filters-section">
      ...
      <div class="filter-controls">
        <select v-model="statusFilter" class="filter-select" @change="fetchArticles">
          <option value="">All Status</option>
          <option value="published">Published</option>
          <option value="draft">Drafts</option>
        </select>

        <select v-model="sortBy" class="filter-select" @change="fetchArticles">
      ...
      <div v-else class="articles-table">
        ...
        >
          ...
          <div class="col-status">
            <span
              class="status-badge"
              :class="{ published: article.is_published, draft: !article.is_published }"
            >
...
async function fetchArticles() {
  ...
  try {
    const params = {
      ...
      limit: 20,
      published: statusFilter.value === 'published' ? 1 : statusFilter.value === 'draft' ? 0 : undefined
    }
...
async function fetchStats() {
  try {
    ...
    if (tagsResponse.data.success) {
      stats.value.totalTags = tagsResponse.data.pagination?.total_count || tagsResponse.data.tags?.length || 0
    }
  } catch (err) {
...
async function deleteArticle() {
  ...
  try {
    const response = await axios.delete(`/api/admin/articles/${deleteConfirmation.value.id}`)

    if (response.data.success) {
      // Remove from list
      articles.value = articles.value.filter(a => a.id !== deleteConfirmation.value.id)
      deleteConfirmation.value = null
